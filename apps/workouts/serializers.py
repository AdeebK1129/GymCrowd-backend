...
3. `UserWorkoutSerializer` - Serializes a user's workout session, including nested 
   exercises and user details.

Dependencies:
- `rest_framework.serializers`: Provides base classes for defining serializers.
- Other serializers (`UserSerializer`) are imported to handle nested user representations.
...
class UserWorkoutSerializer(serializers.ModelSerializer):
    ...
    including nested representations of the associated user and the exercises performed 
    during the workout.

    Attributes:
        user (UserSerializer): A nested serializer for the `User` model, included as a 
            read-only field to display user details in the response.
    ...
    Meta:
        ...
        fields (list[str]): A list of fields to include in the serialized representation:
            ...
            - `user`: Nested details of the associated user.
